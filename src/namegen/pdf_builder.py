...
def build_name_pdf(name_data: list[dict], dest: Path | io.BytesIO | None = None) -> bytes | None:
    ...
    buf = io.BytesIO()
    out: str | io.BytesIO = buf

    BORDER = colors.HexColor(_BORDER_HEX)
    HEADER_BG = colors.HexColor(_HEADER_BG_HEX)
    ROW_ALT = colors.HexColor(_ROW_ALT_HEX)
    ...
    if has_multiple_regions:
        code_legend = " · ".join(f"{region_to_abbr[r]} = {r}" for r in regions)
    ...
    if has_multiple_regions:
        ...
        style_cmds.append(("ALIGN", (5, 0), (5, -1), "CENTER"))
        style_cmds.append(("FONTSIZE", (2, 0), (2, 0), 8))
        style_cmds.append(("FONTSIZE", (5, 0), (5, 0), 8))
        style_cmds.append(("FONTSIZE", (2, 1), (2, -1), 8))
        style_cmds.append(("FONTSIZE", (5, 1), (5, -1), 8))
    for col in separator_columns:
        style_cmds.append(("LINEAFTER", (col, 0), (col, -1), 1.0, BORDER))
